...
const [taskId, tasksDir, taskStr, cwd, timeoutMs, model] = process.argv.slice(2);

if (!taskId || !tasksDir || !taskStr) {
  console.error('Usage: supervisor.mjs <taskId> <tasksDir> <task> [cwd] [timeoutMs] [model]');
  process.exit(1);
}
...
const MODEL = model || ''; // 可选：覆盖默认模型（agent-default-model.config.model）
const PROVIDER = process.env.DSH_DEFAULT_PROVIDER || 'deepseek-official';
...
async function main() {
  ...
  // 更新状态为 running（保留 bridge 写入的 model/cwd 等字段）
  const prevMeta = await readJSON('meta.json') || {};
  await writeJSON('meta.json', {
    ...prevMeta, taskId, status: 'running', startedAt: Date.now(), updatedAt: Date.now(), eventSeq: 1, pid: process.pid,
    ...(MODEL ? { model: MODEL } : {}),
  });
  await appendEvent({ seq: 1, event: 'status', status: 'running', timestamp: Date.now() });

  // 启动 DSH headless
  // 关键：不用 dsh.cmd (cmd.exe 会把中文/引号用 GBK 破坏)，直接用 node 跑 bin.js
  // node → node 之间通过 UTF-16 argv 传参，零编码损失
  const dshBinJs = process.env.DSH_BIN_JS || (
    process.platform === 'win32'
      ? join(process.env.APPDATA || '', 'npm', 'node_modules', '@deepseek-ai', 'dsh', 'lib', 'bin.js')
      : null // 非 Windows 直接调用 dsh
  );

  // 如果指定了模型，生成临时 patch 覆盖 agent-default-model
  let patchPath = null;
  if (MODEL) {
    patchPath = join(TASK_DIR, 'model-patch.yml');
    await writeFile(patchPath, [
      '# auto-generated model override',
      '- id: agent-default-model',
      '  config:',
      `    provider: ${PROVIDER.replace(/[^\w-]/g, '')}`,
      `    model: ${MODEL.replace(/[^\w.\-/]/g, '')}`,
      '',
    ].join('\n'), 'utf-8');
    await appendEvent({ seq: 2, event: 'status', status: `model: ${MODEL}`, timestamp: Date.now() });
  }

  const headlessArgs = ['--profile', 'headless'];
  if (patchPath) { headlessArgs.push('--patch', patchPath); }
  headlessArgs.push(taskStr);

  const child = dshBinJs
    ? spawn(process.execPath, [dshBinJs, ...headlessArgs], {
        cwd: WORK_DIR,
        shell: false,
        stdio: ['ignore', 'pipe', 'pipe'],
        env: { ...process.env, DSH_NO_COLOR: '1', FORCE_COLOR: '0' },
      })
    : spawn('dsh', headlessArgs, {
        cwd: WORK_DIR,
        shell: false,
        stdio: ['ignore', 'pipe', 'pipe'],
        env: { ...process.env, DSH_NO_COLOR: '1', FORCE_COLOR: '0' },
  ...
  await writeJSON('supervisor.json', {
    taskId, pid, dshBin: dshBinJs || 'dsh', cwd: WORK_DIR, spawnedAt: Date.now(), hostname: hostname(),
  });
