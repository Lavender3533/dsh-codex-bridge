...
function getDshBin() {
  if (process.env.DSH_PATH) return process.env.DSH_PATH;
  if (process.platform === 'win32') {
    const paths = (process.env.PATH || '').split(';');
    for (const p of paths) {
      const candidate = join(p.trim(), 'dsh.cmd');
      try { accessSync(candidate); return candidate; } catch {}
    }
    // Fallback: use cmd /c to run dsh
    return 'dsh';
  }
  return 'dsh';
}
...
class Supervisor {
  ...
  async startTask(taskId, request) {
    ...
    const child = spawn(process.execPath, [
      ...
      shell: false,
      stdio: 'ignore',
      env: { ...process.env, DSH_PATH: dshBin, DSH_NO_COLOR: '1', FORCE_COLOR: '0' },
      detached: true,
    });
    child.unref();
    ...
    await this.store.write(taskId, 'supervisor.json', {
      taskId, pid: child.pid, dshBin, cwd: resolve(request.cwd || homedir()),
