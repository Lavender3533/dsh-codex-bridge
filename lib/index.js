...
const ALLOWED_CWD = [homedir(), 'D:/project', 'D:\\project'];
...
function getDefaultProvider() {
  // 从 settings.yaml 读 agent-default-model 段的 provider（模型覆盖 patch 需要）
  try {
    const raw = readFileSync(join(homedir(), '.dsh', 'settings.yaml'), 'utf-8');
    const m = raw.match(/agent-default-model:\s*\n(?:[ \t]+[^\n]+\n)*?[ \t]+provider:\s*(\S+)/);
    if (m) return m[1];
  } catch {}
...
class Supervisor {
  ...
  async startTask(taskId, request) {
    ...
    const child = spawn(process.execPath, [
      ...
      shell: false,
      stdio: 'ignore',
      env: { ...process.env, ...(dshBinJs ? { DSH_BIN_JS: dshBinJs } : {}), DSH_DEFAULT_PROVIDER: getDefaultProvider(), DSH_NO_COLOR: '1', FORCE_COLOR: '0' },
      detached: true,
    });
    child.unref();
...
export function apply(ctx) {
  ...
  server.tool('dsh_task_start', 'Start an async DSH task. Returns taskId immediately.', {
    ...
    model: z.string().optional().describe('Model ID to use (e.g. deepseek-v4-flash-0731); default: profile default model'),
  }, async (args) => {
    const { taskId } = args;
    ...
    await store.write(taskId, 'meta.json', { taskId, status: TASK_STATUS.QUEUED, createdAt: Date.now(), updatedAt: Date.now(), eventSeq: 0, cwd: request.cwd, model: args.model || '' });
